...
for n in range(1,len(model)-1):
    ...
    x_dim = round(float(parse_dim[0]))
    y_dim = round(float(parse_dim[1]))

    position_str = link[2].text
    parse_pos = position_str.split(' ')
    x_pos = int(round(float(parse_pos[0])))+2
    y_pos = int(round(float(parse_pos[1])))-4
    ...
    if angle in x_angle:
        for i in range(int(x_pos-(round(x_dim/2)+1)), int(x_pos+(round(x_dim/2)+1))):
            for j in range(int(y_pos-(round(y_dim/2)+1)), int(y_pos+(round(y_dim/2)+1))):
                obstacle_list.append((i,j))
                if flag:
                    test_list.append((i,j))
    else:
        for j in range(int(y_pos-(round(x_dim/2)+1)), int(y_pos+(round(x_dim/2)+1))):
            for i in range(int(x_pos-(round(y_dim/2)+1)), int(x_pos+(round(y_dim/2)+1))):
                obstacle_list.append((i,j))
                if flag:
                    test_list.append((i,j))
